  ...
  return (
    <nav className="z-10 w-full bg-zinc-800 text-white lg:bg-transparent lg:absolute">
      <div className="container flex items-center mx-auto px-6 py-4 md:px-10 md:flex-col md:gap-2 lg:flex-row lg:gap-0">
        ...
        <div className="hidden z-20 absolute md:flex flex-col w-56 h-dvh top-0 right-0 text-center bg-zinc-900 md:bg-transparent text-white ml-auto md:static md:flex-row md:w-max md:h-auto md:mx-auto lg:mx-0 lg:ml-auto" ref={menuList}>
        ...
        <div className="hidden md:hidden z-10 absolute top-0 left-0 w-full h-dvh bg-black opacity-75" onClick={handleMenu} ref={menuShadow}></div>
      </div>
    </nav>
  );
}
