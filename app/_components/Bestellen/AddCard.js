...
export default function AddCard(props) {
  ...
  function addHandler() {
    if(oneVersion == true) {
      ...
      router.push("/bestellen/product/" + product.id)
    }
  }

  return (
    <div className="flex items-center w-full border-y border-zinc-100 p-2 gap-x-2">
      <Link href={"/bestellen/product/" + product.id} className="h-12 w-12 aspect-square rounded-full border-2"></Link>
      <div>
        <h2>{product.name}</h2>
        <span className="text-xs text-zinc-600">💶 {formattedPrices}</span>
      </div>
      <button className="flex items-center gap-2 ml-auto rounded-full font-medium bg-zinc-200 px-4 py-2 active:scale-95 transition" onClick={addHandler}>
