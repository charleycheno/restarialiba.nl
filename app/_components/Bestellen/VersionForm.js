...
export default function VersionList(props) {
   ...
   return (
      <>
            ...
            <div className="flex flex-wrap gap-4">
               {
                  versions.map((version) => {
                     return (
                        <button key={versions.indexOf(version)} className={"flex flex-col items-center justify-center flex-grow border-2 rounded-lg p-4" + (selectedVersion == versions.indexOf(version) ? " bg-zinc-200" : "")} onPress={() => selectVersion(versions.indexOf(version))}>
         ...
         <button className="flex items-center justify-center gap-1 bg-blue-400 rounded-full py-3 text-white text-lg font-bold active:scale-95 transition" onPress={addHandler}><svg className="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="#fff" d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"/></svg>Toevoegen</button>
      </>
   )
}
